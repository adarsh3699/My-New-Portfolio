...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
	...
	const heightTransform = useTransform(scrollYProgress, [-0.07, 1], [0, height]);
				...
				{data.map((item, index) => (
					<div key={index} className="flex justify-start pt-10 md:pt-30 md:gap-0">
						<div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
							...
							<div className="hidden md:block text-3xl md:pl-18 lg:text-5xl font-bold text-neutral-500 dark:text-neutral-500 ">
								{item.title}
							</div>
						</div>
